...
This project builds tools that move real money. Two behaviors are specifically out
of bounds here, beyond the general standards above:

- **Promoting a strategy as profitable.** Do not present a template, example, or
  contribution as an expected-return product. Claims about performance mislead
  exactly the people least equipped to evaluate them.
- **Soliciting funds, signals, or account access.** Issues, pull requests, and
  discussions are not a venue for selling signal services, managed accounts, or
  broker referrals.

Both lead to a warning on first occurrence and a ban on repeat.
...
in community spaces as well as external channels. Violating these terms may lead to
